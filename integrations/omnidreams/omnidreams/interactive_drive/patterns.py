...
def resample_polyline(
    ...
    for distance in sample_distances:
        ...
        point = (
            points_xyz[segment_index] * (1.0 - local_t)
            + points_xyz[segment_index + 1] * local_t
        )
...
def _segment_lengths(
    ...
    lengths = np.linalg.norm(line_segments[:, 1] - line_segments[:, 0], axis=1).astype(
        np.float32
    )
    cumulative = np.concatenate(
        [np.zeros((1,), dtype=np.float32), np.cumsum(lengths, dtype=np.float32)]
...
def offset_segments(
    ...
    if not left:
        empty = np.empty((0, 2, 3), dtype=np.float32)
        return empty, empty
    return np.stack(left, axis=0).astype(np.float32), np.stack(right, axis=0).astype(
        np.float32
    )
...
def triangulate_polygon_fan(
    ...
    if (
        len(unique_points) >= 2
        and np.linalg.norm(unique_points[0] - unique_points[-1]) < 1e-4
    ):
    ...
    triangles = [
        np.stack([base, unique_points[i], unique_points[i + 1]], axis=0).astype(
            np.float32
        )
