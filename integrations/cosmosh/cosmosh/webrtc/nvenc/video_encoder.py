...
frames for. Two implementations live in this module:
  ...
  * :class:`PyNvHardwareEncoder` — GPU-accelerated NVENC H.264 via
    :class:`~cosmosh.webrtc.nvenc.encoder.CosmoshNvencH264`. Owns the
    NAL queue drained by the aiortc adapter
    (:mod:`cosmosh.webrtc.nvenc.aiortc_compat`), installs / uninstalls
  ...
  * :class:`DefaultRTCVideoEncoder` — aiortc's built-in software
    encoder path. Wraps a :class:`~cosmosh.webrtc.media.CosmoshVideoTrack`
...
Session managers pick one concrete implementation at initialisation
and use it through the Protocol thereafter — encoder-specific
branching lives here, nowhere else.

Naming and Protocol shape mirror the sister flashdreams / omnidreams
refactor (``0001-Refactor-WebRTC-encoder-into-VideoEncoder-abstractio.patch``)
so cross-project engineers see the same abstraction on both sides.
"""
...
from cosmosh.webrtc.nvenc.aiortc_compat import (
    install_nvenc_encoder,
    restrict_codecs_to_h264,
    uninstall_nvenc_encoder,
)
from cosmosh.webrtc.nvenc.encoder import CosmoshNvencH264, NalFrame, NvencConfig
from cosmosh.webrtc.nvenc.resolver import probe_nvenc_available
...
class PyNvHardwareEncoder:
    ...
    :class:`~cosmosh.webrtc.nvenc.track.CosmoshNvencVideoTrack`
    ...
    def is_supported(
        ...
        *,
        ...
        :func:`~cosmosh.webrtc.nvenc.resolver.probe_nvenc_available`
    ...
    def create_track(self) -> MediaStreamTrack:
        ...
        from cosmosh.webrtc.nvenc.track import CosmoshNvencVideoTrack
    ...
    async def deliver_chunk(
        ...
        *,
        ...
        from cosmosh.webrtc.nvenc.pack import denormalize_and_pack_argb
        from cosmosh.webrtc.nvenc.track import CosmoshNvencVideoTrack
...
class DefaultRTCVideoEncoder:
    ...
    :class:`~cosmosh.webrtc.media.CosmoshVideoTrack` that carries raw
