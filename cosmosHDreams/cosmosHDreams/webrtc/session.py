...
from cosmosHDreams.webrtc.controls_quest import (
    VRControllerState,
...
from cosmosHDreams.config import COSMOSHDREAMS_RUNNERS
...
class _LatencyLogger:
    ...
    def __init__(self, mode: str) -> None:
        ...
        self._path = f"cosmosHDreams_perf_{ts}.jsonl"
...
class CosmoshRuntimeConfig:
    config_name: str = "cosmosHDreams-lightvae-lighttae"
...
class CosmoshInferenceRuntime:
    ...
    def _generate_one_chunk_debug_sync(self) -> CosmoshStepResult | None:
        ...
        Mirrors ``cosmosHDreams.webrtc.replay`` exactly: the recorded actions go
    ...
    def _initialize_sync(self) -> None:
        ...
        if self.config.config_name not in COSMOSHDREAMS_RUNNERS:
            supported = ", ".join(sorted(COSMOSHDREAMS_RUNNERS))
            raise ValueError(
        ...
        # explicit resolution.
        cond_frame = load_conditional_frame(
        ...
        runner_cfg = COSMOSHDREAMS_RUNNERS[self.config.config_name]
    ...
    def _set_scene_sync(self, scene: Scene) -> None:
        ...
        cond_frame = load_conditional_frame(scene.input_path, scene.start_frame_idx)
        if (cond_frame.shape[0], cond_frame.shape[1]) != (ht, wt):
            cond_frame = mediapy.resize_image(cond_frame, (ht, wt))
        cond_pixels = pixel_frame_to_neg1_pos1(
            cond_frame, device=self._device, dtype=self._dtype
