...
session-level chunk timing in :mod:`cosmosHDreams.webrtc.session`."""
...
def tensor_chunk_to_rgb_frames(video_chunk: torch.Tensor) -> list[np.ndarray]:
    ...
    anchor) in the same ``[B, C, T, H, W]`` layout used by run_cosmosHDreams.py's
    final video buffer. Mapping mirrors run_cosmosHDreams.py:506-513.
    """
