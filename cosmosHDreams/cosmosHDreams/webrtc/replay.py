...
``flashdreams-run cosmosHDreams-*`` runner — if the runner produces good video but
...
``cosmosHDreams-chunk1-*`` -> 1, ``cosmosHDreams-chunk2-*`` -> 2, ``cosmosHDreams-chunk3-*`` -> 3.

Usage:
    python -m cosmosHDreams.webrtc.replay \\
        ...
        --config-name cosmosHDreams-chunk3-vae-vae \\
...
from cosmosHDreams.runner import _annotate_frame_numbers
from cosmosHDreams.webrtc.session import CosmoshInferenceRuntime, CosmoshRuntimeConfig
from cosmosHDreams.webrtc.utils import ACTION_DIM_NORMALISED

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s %(name)s: %(message)s",
)
LOGGER = logging.getLogger("cosmosHDreams.replay")
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--config-name",
        default="cosmosHDreams-lightvae-lighttae",
        help="COSMOSHDREAMS_RUNNERS slug (len_t is baked into the chunk variants; "
        "e.g. cosmosHDreams-chunk3-vae-vae). Default: cosmosHDreams-lightvae-lighttae.",
    )
