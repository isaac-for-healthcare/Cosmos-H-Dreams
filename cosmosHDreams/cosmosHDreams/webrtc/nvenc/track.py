...
Drop-in replacement for :class:`cosmosHDreams.webrtc.media.CosmoshVideoTrack`
when ``video.encoder=nvenc``. Differences from the CPU track:
  ...
  - The track does **not** carry pixel data. NAL units flow through a
    ...
    (:mod:`cosmosHDreams.webrtc.nvenc.aiortc_compat`). This track only
...
Lockstep contract with :mod:`cosmosHDreams.webrtc.nvenc.aiortc_compat`:
...
# Same thresholds as :mod:`cosmosHDreams.webrtc.media`; keeping them in sync
