...
"""CosmosH DiT building blocks (single-view, no cross-view attention)."""
...
class Mlp(nn.Module):
    """Two-layer MLP used by the action embedder.

    Mirrors the ``Mlp`` shape in the upstream
    ``ActionChunkConditionedMinimalV1LVGDiT`` so checkpoint keys load 1:1.
    """
...
class Block(nn.Module):
    """CosmosH transformer block with self-attn, cross-attn, and MLP branches.

    Single-view: tensors are 3D ``[B, L, D]`` where ``L = T*H*W`` (already
    flattened by the network's patchify pass).
    """
    ...
    def set_context_parallel_group(
        ...
        """Set the CP group for self-attention (T-axis gathering)."""
    ...
    def initialize_cache(
        ...
        Args:
            chunk_size: Tokens appended to the self-attention KV per AR step.
            window_size: Self-attention rolling-window capacity in tokens.
            sink_size: Self-attention sink-token capacity.
            context: Cross-attention context, shape ``[B, L_ctx, D_ctx]``.
        """
