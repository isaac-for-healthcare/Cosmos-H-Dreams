...
"""Streaming inference pipeline for Cosmos-H-Dreams (single-view, action-conditioned).
...
from cosmosHDreams.encoder.action import (
    ActionEncoderCache,
    ActionEncoderConfig,
    num_generated_frames,
)
from cosmosHDreams.transformer import (
    CosmosHTransformer,
...
class CosmoshPipelineConfig(StreamInferencePipelineConfig):
    """Config for the Cosmos-H-Dreams pipeline.
...
class CosmoshPipeline(
    ...
    """Cosmos-H-Dreams streaming inference pipeline (single-view, action-conditioned)."""
